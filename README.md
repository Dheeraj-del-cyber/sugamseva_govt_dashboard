...
│   │   ├── services/      biometric, digilocker, ocr, ai (Claude), notify (SMS/FCM)
...
| Document verification | DigiLocker + APISetu (mocked), PaddleOCR (mocked) |
...
- **DigiLocker / APISetu** — needs a registered organisation account and
  OAuth2 client credentials from https://partners.digitallocker.gov.in, plus
  citizen consent flows.
- **Bhashini** — needs an API key from https://bhashini.gov.in.
...
1. Get DigiLocker/APISetu, Bhashini, SMS gateway, and Firebase credentials and
   drop them into `backend/.env`, then set `DEMO_MODE=false`.
