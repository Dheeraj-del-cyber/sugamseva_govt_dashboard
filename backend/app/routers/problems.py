...
def list_problems(
    ...
    if status == "solved":
        ...
        query = query.filter(
            models.Problem.is_solved == False
        )  # noqa: E712
...
@router.get(
    "/{problem_id}",
    response_model=schemas.ProblemDetailOut
)
def get_problem(
    problem_id: str,
    db: Session = Depends(get_db),
...
def update_problem(
    ...
    """Edit a problem's title, description or category.
    Vote counts and solved status are untouched.
    Only the reported details change.
    """
    ...
    if payload.title is not None:
        if not payload.title.strip():
            raise HTTPException(
                status_code=400,
                detail="Problem title cannot be empty"
            )
...
@router.get(
    "/{problem_id}/users",
    response_model=list[schemas.ProblemAffectedUser]
)
...
def vote_problem(
    ...
    citizen = (
        ...
        .filter(models.Citizen.id == payload.citizen_id)
        .first()
    )
    ...
    existing_vote = (
        ...
        .filter(
            models.ProblemVote.problem_id == problem_id,
            models.ProblemVote.citizen_id == payload.citizen_id,
        )
        .first()
    )
    ...
    if existing_vote:
        raise HTTPException(
            status_code=400,
            detail="This citizen has already voted for this problem and it cannot be reverted"
        )

    vote = models.ProblemVote(
    ...
    notify.send_sms(
        ...
        f"Your vote for '{problem.title}' has been recorded. Thank you."
    )
...
def mark_solved(
    ...
    if not biometric.check_verification_token(
        payload.fingerprint_verification_token
    ):
        raise HTTPException(
            status_code=401,
            detail="Citizen fingerprint verification required"
        )
    ...
    if not vote:
        raise HTTPException(
            status_code=404,
            detail="This citizen has not voted for this problem"
        )

    if vote.solved:
        raise HTTPException(
            status_code=400,
            detail="Already marked solved"
        )
    ...
    notify.send_sms(
        citizen.phone_number,
        f"Good news! Your reported issue '{problem.title}' has been marked as solved."
    )
...
def notify_voters(
    ...
    """Send every citizen who voted for this problem an SMS with the
    current status (in progress / fully solved), so officials can
    proactively update citizens without waiting for them to check the app.
    """
