...
function SchemeCatalog() {
  ...
  const categories = Array.from(
    new Set(catalog.map((scheme) => scheme.category).filter(Boolean)),
  );

  const filteredCatalog = catalog.filter((scheme) => {
    ...
    const matchesStatus =
      status === "all" ||
      (status === "active" && scheme.active) ||
      (status === "inactive" && !scheme.active);
    ...
    <div>
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        ...
        <button
          ...
          className="inline-flex items-center justify-center gap-2 rounded-lg border px-4 py-2.5 text-sm font-medium text-ink-700 hover:bg-ink-100"
          style={{ borderColor: "var(--color-ink-300)" }}
        >
